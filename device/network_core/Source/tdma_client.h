...
typedef enum {
    TDMA_CLIENT_UNREGISTERED,  ///< the DotBot is not registered with the gateway
    TDMA_CLIENT_REGISTERED,    ///< the DotBot registered with the gateway
} tdma_registration_state_t;

/// TDMA internal RX state
typedef enum {
    TDMA_CLIENT_RX_WAIT,  ///< the DotBot receiver is OFF
    TDMA_CLIENT_RX_ON,    ///< the DotBot is receiving radio packets
} tdma_rx_state_t;
