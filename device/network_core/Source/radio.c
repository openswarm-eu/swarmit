...
typedef struct __attribute__((packed)) {
    ...
    uint8_t payload[BLE_PAYLOAD_MAX_LENGTH];  ///< Payload + MIC (if any) (BLE_PAYLOAD_MAX_LENGTH > IEEE802154_PAYLOAD_MAX_LENGTH)
} radio_pdu_t;
...
typedef struct {
    ...
    radio_mode_t mode;      ///< PHY protocol used by the radio (BLE, IEEE 802.15.4)
} radio_vars_t;
...
void radio_init(radio_cb_t callback, radio_mode_t mode) {
    ...
    if (mode == RADIO_IEEE802154_250Kbit) {
    ...
    if (mode == RADIO_IEEE802154_250Kbit) {
        ...
        NRF_RADIO->PCNF1 = (IEEE802154_PAYLOAD_MAX_LENGTH << RADIO_PCNF1_MAXLEN_Pos) |  // Max payload of 127 bytes
    ...
    } else if (mode == RADIO_BLE_1MBit || mode == RADIO_BLE_2MBit) {
        ...
        NRF_RADIO->PCNF1 = (4UL << RADIO_PCNF1_BALEN_Pos) |  // The base address is 4 Bytes long
                           (BLE_PAYLOAD_MAX_LENGTH << RADIO_PCNF1_MAXLEN_Pos) |
...
void radio_set_channel(uint8_t channel) {
    uint8_t freq;
    if (radio_vars.mode == RADIO_IEEE802154_250Kbit) {
