...
typedef struct {
    uint8_t  buffer[TDMA_CLIENT_RING_BUFFER_SIZE][BLE_PAYLOAD_MAX_LENGTH];  ///< arrays of radio messages waiting to be sent
...
typedef struct {
    ...
    tdma_registration_state_t registration_flag;                     ///< flag marking if the DotBot is registered with the Gateway or not.
    tdma_rx_state_t           rx_flag;                               ///< flag marking if the DotBot's is receving or not.
...
static const uint8_t ble_mode_to_byte_time[] = {
    8,   // RADIO_BLE_1MBit
    4,   // RADIO_BLE_2MBit
    64,  // RADIO_BLE_LR125Kbit
    16,  // RADIO_BLE_LR500Kbit
};
 ...
 */
static void _message_rb_add(tdma_client_ring_buffer_t *rb, uint8_t data[BLE_PAYLOAD_MAX_LENGTH], uint8_t packet_length);
 ...
 */
static bool _message_rb_get(tdma_client_ring_buffer_t *rb, uint8_t data[BLE_PAYLOAD_MAX_LENGTH], uint8_t *packet_length);
...
void tdma_client_init(tdma_client_cb_t callback, radio_mode_t radio_mode, uint8_t radio_freq) {
    ...
    // Initialize Radio
    radio_init(&tdma_client_callback, radio_mode);  // set the radio callback to our tdma catch function
    radio_set_frequency(radio_freq);                // pass through the rest of the arguments
    radio_rx();                                     // start receiving packets
    ...
    _tdma_client_vars.registration_flag = TDMA_CLIENT_UNREGISTERED;
    _tdma_client_vars.rx_flag           = TDMA_CLIENT_RX_ON;

    // Configure the Timers
    _tdma_client_vars.last_tx_packet_timestamp = timer_hf_now(TDMA_CLIENT_TIMER_HF);                                                  // start the counter saving when was the last packet sent.
    timer_hf_set_oneshot_us(TDMA_CLIENT_TIMER_HF, TDMA_CLIENT_HF_TIMER_CC_TX, TDMA_CLIENT_DEFAULT_TX_START, &timer_tx_interrupt);     // start advertising behaviour
    timer_hf_set_oneshot_us(TDMA_CLIENT_TIMER_HF, TDMA_CLIENT_HF_TIMER_CC_RX, TDMA_CLIENT_DEFAULT_RX_DURATION, &timer_rx_interrupt);  // check RX timer once per frame.
}
...
static void _message_rb_add(tdma_client_ring_buffer_t *rb, uint8_t data[BLE_PAYLOAD_MAX_LENGTH], uint8_t packet_length) {
    ...
    memcpy(rb->buffer[rb->write_index], data, BLE_PAYLOAD_MAX_LENGTH);
...
static bool _message_rb_get(tdma_client_ring_buffer_t *rb, uint8_t data[BLE_PAYLOAD_MAX_LENGTH], uint8_t *packet_length) {
    ...
    memcpy(data, rb->buffer[rb->read_index], BLE_PAYLOAD_MAX_LENGTH);
...
static bool _message_rb_tx_queue(uint16_t max_tx_duration_us) {
    ...
    uint32_t start_tx_slot                     = timer_hf_now(TDMA_CLIENT_TIMER_HF);
    uint8_t  length                            = 0;
    uint8_t  packet[BLE_PAYLOAD_MAX_LENGTH] = { 0 };
    ...
    while (_tdma_client_vars.tx_ring_buffer.count > 0) {
        ...
        if (timer_hf_now(TDMA_CLIENT_TIMER_HF) + tx_time - start_tx_slot < max_tx_duration_us) {

            // disable the radio, before sending.
            radio_disable();
            radio_tx(packet, length);
...
static void tdma_client_callback(uint8_t *packet, uint8_t length) {
    ...
    switch (header->packet_type) {
        case PACKET_TDMA_UPDATE_TABLE:
        {
            ...
            if (_tdma_client_vars.registration_flag == TDMA_CLIENT_UNREGISTERED) {
                _tdma_client_vars.registration_flag = TDMA_CLIENT_REGISTERED;
            }

            // Update the state machine
            _tdma_client_vars.rx_flag = TDMA_CLIENT_RX_WAIT;

            // Update the timer interrupts
            timer_hf_set_oneshot_us(TDMA_CLIENT_TIMER_HF, TDMA_CLIENT_HF_TIMER_CC_TX, next_period_start + _tdma_client_vars.tdma_client_table.tx_start, &timer_tx_interrupt);
            timer_hf_set_oneshot_us(TDMA_CLIENT_TIMER_HF, TDMA_CLIENT_HF_TIMER_CC_RX, next_period_start + _tdma_client_vars.tdma_client_table.rx_start, &timer_rx_interrupt);
        ...
        case PACKET_TDMA_SYNC_FRAME:
        {
            ...
            if (_tdma_client_vars.registration_flag == TDMA_CLIENT_REGISTERED) {
                ...
                // Update the timer interrupts
                timer_hf_set_oneshot_us(TDMA_CLIENT_TIMER_HF, TDMA_CLIENT_HF_TIMER_CC_TX, _tdma_client_vars.tdma_client_table.tx_start - tx_time, &timer_tx_interrupt);
                timer_hf_set_oneshot_us(TDMA_CLIENT_TIMER_HF, TDMA_CLIENT_HF_TIMER_CC_RX, _tdma_client_vars.tdma_client_table.rx_start - tx_time, &timer_rx_interrupt);
...
static void timer_tx_interrupt(void) {
    ...
    if (_tdma_client_vars.registration_flag == TDMA_CLIENT_REGISTERED) {
        ...
        timer_hf_set_oneshot_us(TDMA_CLIENT_TIMER_HF, TDMA_CLIENT_HF_TIMER_CC_TX, _tdma_client_vars.tdma_client_table.frame_duration, &timer_tx_interrupt);
        ...
        if (!packet_sent) {
            if (timer_hf_now(TDMA_CLIENT_TIMER_HF) - _tdma_client_vars.last_tx_packet_timestamp > TDMA_CLIENT_MAX_DELAY_WITHOUT_TX) {

                _tx_keep_alive_message();
                // Save the timestamp of the last packet
                _tdma_client_vars.last_tx_packet_timestamp = timer_hf_now(TDMA_CLIENT_TIMER_HF);
            }
        }
        ...
        timer_hf_set_oneshot_us(TDMA_CLIENT_TIMER_HF, TDMA_CLIENT_HF_TIMER_CC_TX, delay_time, &timer_tx_interrupt);
...
static void timer_rx_interrupt(void) {
    ...
    if (_tdma_client_vars.rx_flag == TDMA_CLIENT_RX_WAIT) {
        // Set the next interruption
        timer_hf_set_oneshot_us(TDMA_CLIENT_TIMER_HF, TDMA_CLIENT_HF_TIMER_CC_RX, _tdma_client_vars.tdma_client_table.rx_duration, &timer_rx_interrupt);
        // turn the radio ON
        radio_rx();
    } else if (_tdma_client_vars.rx_flag == TDMA_CLIENT_RX_ON) {
        // Set the next interruption
        uint32_t delay = _tdma_client_vars.tdma_client_table.frame_duration - _tdma_client_vars.tdma_client_table.rx_duration;
        timer_hf_set_oneshot_us(TDMA_CLIENT_TIMER_HF, TDMA_CLIENT_HF_TIMER_CC_RX, delay, &timer_rx_interrupt);
        // turn the radio OFF
        radio_disable();
    }
}
