...
size_t db_hdlc_encode(const uint8_t *input, size_t input_len, uint8_t *frame) {
    ...
    // Start flag
    frame[frame_len++] = HDLC_FLAG;

    for (uint8_t pos = 0; pos < input_len; pos++) {
        uint8_t byte = input[pos];
        fcs          = _db_hdlc_update_fcs(fcs, byte);
        if (byte == HDLC_ESCAPE) {
            frame[frame_len++] = HDLC_ESCAPE;
            frame[frame_len++] = HDLC_ESCAPE_ESCAPED;
        } else if (byte == HDLC_FLAG) {
            frame[frame_len++] = HDLC_ESCAPE;
            frame[frame_len++] = HDLC_FLAG_ESCAPED;
        } else {
    ...
    if ((fcs & 0xFF) == HDLC_ESCAPE) {
        frame[frame_len++] = HDLC_ESCAPE;
        frame[frame_len++] = HDLC_ESCAPE_ESCAPED;
    } else if ((fcs & 0xFF) == HDLC_FLAG) {
        frame[frame_len++] = HDLC_ESCAPE;
        frame[frame_len++] = HDLC_FLAG_ESCAPED;
    } else {
        frame[frame_len++] = (fcs & 0xFF);
    }
    if (((fcs & 0xFF00) >> 8) == HDLC_ESCAPE) {
        frame[frame_len++] = HDLC_ESCAPE;
        frame[frame_len++] = HDLC_ESCAPE_ESCAPED;
    } else if (((fcs & 0xFF00) >> 8) == HDLC_FLAG) {
        frame[frame_len++] = HDLC_ESCAPE;
        frame[frame_len++] = HDLC_FLAG_ESCAPED;
    } else {
