...
typedef struct {
    uint8_t  buffer[TDMA_RING_BUFFER_SIZE][BLE_PAYLOAD_MAX_LENGTH];  ///< arrays of radio messages waiting to be sent
 ...
 */
static void _message_rb_add(tdma_ring_buffer_t *rb, uint8_t data[BLE_PAYLOAD_MAX_LENGTH], uint8_t packet_length);
 ...
 */
static bool _message_rb_get(tdma_ring_buffer_t *rb, uint8_t data[BLE_PAYLOAD_MAX_LENGTH], uint8_t *packet_length);
...
void tdma_server_init(tdma_server_cb_t callback, radio_mode_t radio_mode, uint8_t radio_freq, application_type_t default_radio_app) {

    // Initialize high frequency clock
    timer_hf_init(TDMA_SERVER_TIMER_HF);
    ...
    // Initialize Radio
    radio_init(&tdma_server_callback, radio_mode);  // set the radio callback to our tdma catch function
    ...
    _tdma_vars.last_tx_packet_ts = timer_hf_now(TDMA_SERVER_TIMER_HF);                                                                                                    // start the counter saving when was the last packet sent.
    _tdma_vars.frame_start_ts    = _tdma_vars.last_tx_packet_ts;                                                                                                             // start the counter saving when was the last packet sent.
    timer_hf_set_periodic_us(TDMA_SERVER_TIMER_HF, TDMA_SERVER_HF_TIMER_CC, _tdma_vars.tdma_table.table[_tdma_vars.active_slot_idx].tx_duration, &timer_tdma_interrupt);  // start advertising behaviour
}

void tdma_server_get_table_info(uint32_t *frame_duration_us, uint16_t *num_clients, uint16_t *table_index) {
...
static bool _message_rb_tx_queue(tdma_ring_buffer_t *rb, uint16_t max_tx_duration_us) {
    ...
    while (rb->count > 0) {
        ...
        if (timer_hf_now(TDMA_SERVER_TIMER_HF) + tx_time - _tdma_vars.slot_start_ts < max_tx_duration_us) {
...
static void _tx_sync_frame(void) {
    ...
    radio_disable();
    radio_tx(_tdma_vars.radio_buffer, length);
}
...
static void _tx_registration_messages(uint64_t client) {
    ...
    protocol_header_to_buffer(_tdma_vars.radio_buffer, client, _tdma_vars.default_radio_app, PROTOCOL_TDMA_UPDATE_TABLE);

    // Compute the time before the next frame. (as close as possible to the TX as you can, so that it's more accurate)
    table.next_period_start = table.frame_period - (timer_hf_now(TDMA_SERVER_TIMER_HF) - _tdma_vars.frame_start_ts);
...
static void tdma_server_callback(uint8_t *packet, uint8_t length) {
    ...
    */
    ...
    if (header->version != (uint8_t)FIRMWARE_VERSION) {
        return;
    }
    ...
    if (header->type == PROTOCOL_TDMA_KEEP_ALIVE) {
        return;
    }
...
static void timer_tdma_interrupt(void) {
    ...
    _tdma_vars.slot_start_ts = timer_hf_now(TDMA_SERVER_TIMER_HF);
    ...
    if (_tdma_vars.tdma_table.table[_tdma_vars.active_slot_idx].client == _tdma_vars.device_id) {
        ...
        uint32_t remaining_slot_time_us = _tdma_vars.slot_start_ts + _tdma_vars.tdma_table.table[_tdma_vars.active_slot_idx].tx_duration - timer_hf_now(TDMA_SERVER_TIMER_HF);
        ...
        remaining_slot_time_us = _tdma_vars.slot_start_ts + _tdma_vars.tdma_table.table[_tdma_vars.active_slot_idx].tx_duration - timer_hf_now(TDMA_SERVER_TIMER_HF);
        ...
        if (packet_sent) {
            _tdma_vars.last_tx_packet_ts = timer_hf_now(TDMA_SERVER_TIMER_HF);
        }
    }
}
