...
void tdma_server_init(tdma_server_cb_t callback, radio_mode_t radio_mode, uint8_t radio_freq) {
...
static void _tx_sync_frame(void) {
    ...
    size_t length = protocol_tdma_sync_frame_to_buffer(_tdma_vars.radio_buffer, BROADCAST_ADDRESS, &frame);
    radio_disable();
...
static void _tx_registration_messages(uint64_t client) {
    ...
    size_t length = protocol_tdma_table_update_to_buffer(_tdma_vars.radio_buffer, client, &table);
...
static void tdma_server_callback(uint8_t *packet, uint8_t length) {
    ...
    */
    ...
    if (header->packet_type == PACKET_TDMA_KEEP_ALIVE) {
        return;
    }
