...
void timer_hf_init(timer_hf_t timer) {
...
uint32_t timer_hf_now(timer_hf_t timer) {
    _devs[timer].p->TASKS_CAPTURE[_devs[timer].cc_num] = 1;
    return _devs[timer].p->CC[_devs[timer].cc_num];
}

void timer_hf_set_periodic_us(timer_hf_t timer, uint8_t channel, uint32_t us, timer_hf_cb_t cb) {
...
void timer_hf_set_oneshot_us(timer_hf_t timer, uint8_t channel, uint32_t us, timer_hf_cb_t cb) {
    assert(channel >= 0 && channel < _devs[timer].cc_num + 1);  // Make sure the required channel is correct
    assert(cb);                                                 // Make sure the callback function is valid
...
void timer_hf_set_oneshot_ms(timer_hf_t timer, uint8_t channel, uint32_t ms, timer_hf_cb_t cb) {
    timer_hf_set_oneshot_us(timer, channel, ms * 1000UL, cb);
}

void timer_hf_set_oneshot_s(timer_hf_t timer, uint8_t channel, uint32_t s, timer_hf_cb_t cb) {
    timer_hf_set_oneshot_us(timer, channel, s * 1000UL * 1000UL, cb);
}

void timer_hf_delay_us(timer_hf_t timer, uint32_t us) {
    _devs[timer].p->TASKS_CAPTURE[_devs[timer].cc_num] = 1;
...
void timer_hf_delay_ms(timer_hf_t timer, uint32_t ms) {
    timer_hf_delay_us(timer, ms * 1000UL);
}

void timer_hf_delay_s(timer_hf_t timer, uint32_t s) {
    timer_hf_delay_us(timer, s * 1000UL * 1000UL);
}
